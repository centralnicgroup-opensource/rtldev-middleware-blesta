...
class Cnr extends RegistrarModule
{
    ...
    public function __construct()
    {
        ...
        Loader::load(implode(DS, [__DIR__, "apis", "cnr_logger.php"]));
        // Library classes, in dependency order: CnrDomainManager extends CnrBase.
        $path = implode(DS, [__DIR__, "lib", ""]);
        Loader::load($path . "base.php");
        Loader::load($path . "enums.php");
        Loader::load($path . "settings.php");
        Loader::load($path . "zone_info.php");
        Loader::load($path . "helper.php");
        Loader::load($path . "domain_manager.php");
        Loader::load($path . "additional_fields.php");
        // ---------------------------------------------------------------
        $this->domainManager = new CnrDomainManager();
    }
    ...
    public function validateService($package, array $vars = null)
    {
        ...
        if (($package->meta->type ?? null) !== "domain") {
            return true;
        }

        $this->Input->setRules($this->getServiceRules());

        return $this->Input->validates($vars);
    }

    /**
     * Rules for the fields collected before a domain order is paid for.
     *
     * Blesta runs these at the order configuration step, so anything rejected
     * here never reaches an invoice. Only inputs the registry is certain to
     * refuse are checked: being stricter than the registry would turn orders
     * that register perfectly well today into lost sales.
     *
     * The transfer auth code is deliberately absent. Not every registry uses
     * one - .uk transfers move by tag change - so requiring it would block
     * transfers that succeed without it.
     *
        ...
        $rules = [
            "domain" => [
                "empty" => [
                    "rule" => "isEmpty",
                    "negate" => true,
                    // Without this a blank domain would also fail the format
                    // rule below and report the same problem twice.
                    "last" => true,
                    "message" => Language::_("Cnr.!error.domain.empty", true),
                ],
                "format" => [
                    "rule" => static fn($domain): bool => CnrHelper::isHostName($domain),
                    "message" => Language::_("Cnr.!error.domain.format", true),
                ],
            ],
        ];

        // Nameservers are optional; the managed DNS defaults apply when they
        // are left blank. The field list is read from the configuration so that
        // adding a sixth nameserver there does not silently go unvalidated.
        foreach (array_keys((array) Configure::get("Cnr.nameserver_fields")) as $field) {
            $rules[$field] = [
                "format" => [
                    "if_set" => true,
                    "rule" => static fn($host): bool => trim((string) $host) === ""
                        || CnrHelper::isHostName($host),
                    "message" => Language::_("Cnr.!error.ns.format", true),
                ],
            ];
        }
    ...
    public function addService(
        ...
        CnrBase::setModule($row);
        CnrBase::moduleInstance($this);
        ...
        if (isset($vars["use_module"]) && $vars["use_module"] == "true") {
            if ($package->meta->type === "domain") {
                ...
                if ($zoneInfo->transfer->supportsPreCheck && !empty($vars["transfer_key"])) {
                    ...
                    CnrHelper::errorHandler($r, "/^(200|218)$/");
                    // Handling api errors
                    ...
                    CnrHelper::errorHandler($getZoneInfo);
    ...
    {
        // A cancelled domain is left to lapse: the renewal mode stops it being
        // renewed, and it stays registered until it expires so the customer can
        // still be given it back. Nothing here deletes it at the registry.
        if (($package->meta->type ?? null) !== "domain") {
            return null;
        }
        ...
        CnrHelper::errorHandler($this->domainManager->setDomainRenewalMode(
            $fields->domain,
            CnrRenewalMode::AutoExpire->value
        ));
    ...
    {
        // clientHold takes the domain out of DNS, which is what suspending is
        // meant to do, and unsuspendService() lifts it again. Changing the
        // renewal mode instead would leave the site up and only take effect at
        // expiry - and it is not something unsuspending can undo.
        return $this->setDomainHold($package, $service, true);
    }
    ...
    public function renewService($package, $service, $parent_package = null, $parent_service = null, $years = null)
    {
        ...
        $zoneInfo = $this->domainManager->getZoneInfo($vars["domain"]);
        // Some zones report no information at all. Renewing explicitly is the
        // safe assumption there: the registry rejects the command if the zone
        // does not support it, whereas guessing the renewal-mode path would
        // leave the domain to expire.
        if ($zoneInfo instanceof CnrZoneInfo && $zoneInfo->renewal->requiresRenewalModeFallback()) {
            $r = $this->domainManager->setDomainRenewalMode($vars["domain"], CnrRenewalMode::RenewOnce->value);
            if (!CnrHelper::errorHandler($r)) {
                return;
            }
            return true;
        }

        // EXPIRATION is the registry's guard against renewing the same domain
        // twice: it only accepts the command when the year matches what the
        // domain is currently paid until. It has to come from PAIDUNTILDATE
        // rather than the module's own expiry date, which reports the earlier
        // of the paid-until and renewal dates and would not match.
        // QueryDomainList names this column PAIDUNTIL; StatusDomain calls the
        // same value PAIDUNTILDATE. Reading the wrong one yields null.
        $status = $this->domainManager->getDomainStatusCached($vars["domain"]);
        $paidUntil = $status["PROPERTY"]["PAIDUNTIL"][0] ?? null;
        if ($paidUntil === null) {
            $this->Input->setErrors([
                "errors" => [Language::_("Cnr.!error.renew.no_expiration", true)],
            ]);
            return;
        }
        ...
        $r = $this->domainManager->call([
            ...
            "DOMAIN" => $vars["domain"],
            "PERIOD" => $vars["years"],
            "EXPIRATION" => (int) substr($paidUntil, 0, 4),
        ]);
    ...
    public function getExpirationDate($service, $format = 'Y-m-d H:i:s')
    {
        ...
        $expiryDate = CnrHelper::getExpiryDate(
            $r["DOMAINRENEWALDATE"][0] ?? null,
            // PAIDUNTIL is what QueryDomainList calls it; the previous
            // PAIDUNTILDATE never matched, so this always fell through to the
    ...
    public function manageModule($module, array &$vars)
    {
        ...
        foreach ($module->rows as &$row) {
            $row->meta->connectivity = (bool) CnrHelper::hasCache(base64_encode("user_{$row->meta->user}"));
        }
    ...
    public function addModuleRow(array &$vars, $module_row_id = null)
    {
        ...
        // key => whether the value is stored encrypted
        $meta_fields = [
            "user" => 0,
            "key" => 1,
            "sandbox" => 0,
        ...
        CnrBase::moduleInstance($this);
        $this->Input->setRules(CnrHelper::getRowRules($vars, $module_row_id));
    ...
    {
        return $this->setDomainHold($package, $service, false);
    }

    /**
     * Add or remove the registry hold that suspends a domain.
     *
     * @param stdClass $package The package the service belongs to
     * @param stdClass $service The service to hold or release
     * @param bool $hold True to suspend the domain, false to release it
     * @return null
     */
    private function setDomainHold($package, $service, bool $hold)
    {
        if (($package->meta->type ?? null) !== "domain") {
            return null;
        }
        ...
        CnrHelper::errorHandler($this->domainManager->call([
            "COMMAND" => "ModifyDomain",
            "DOMAIN" => $fields->domain,
            ($hold ? "AddStatus0" : "DelStatus0") => "clientHold",
        ]));
    ...
    public function getAdminAddFields($package, $vars = null)
    {
        ...
        CnrBase::setModule($this->getModuleRow($package->module_row));
        CnrBase::moduleInstance($this);

        $domain = (string) ($vars->domain ?? "");
        $tld = $domain !== "" ? $this->getTld($domain) : "";

        // The TLD specific additional fields can only be resolved once a domain
        // has been entered, and on the first render there is none: this form is
        // where the admin types it. Bailing out here would leave nothing to type
        // into. With no TLD, "Cnr.domain_fields{$tld}" resolves to the base set,
        // which carries the domain field itself.
        if ($tld !== "") {
            Configure::set("Cnr.domain_fields{$tld}", (new CnrAdditionalFields([
                "tld" => $tld,
                "domain" => $domain,
    ...
    public function getClientAddFields($package, $vars = null)
    {
        ...
        if ($tld !== "") {
            Configure::set(
                ...
                (new CnrAdditionalFields([
                    "tld" => ltrim($tld, "."),
                    "domain" => $domain,
                    "type" => isset($vars->transfer) ? "transfer" : "register",
                ]))->getConfiguration()
            );
        }
    ...
    public function getAdminEditFields($package, $vars = null)
    {
        ...
        CnrBase::setModule($this->getModuleRow($package->module_row) ?? $this->getModuleRows()[0]);
        CnrBase::moduleInstance($this);
    ...
    private function manageWhois($view, $package, $service, array $get = null, array $post = null, array $files = null)
    {
        ...
        if ($r["CODE"] === "200") {
            ...
            $additionalFields = new CnrAdditionalFields([
                "tld" => CnrHelper::getSldTld($fields->domain, true),
                "domain" => $fields->domain,
                "type" => "modify"
            ]);
            ...
            foreach ($extension_fields as $key => $value) {
                $vars->{"CnrAdditionalFields-" . $key} = $value;
            }

            $prefixed_extension_fields = [];
            foreach ($extension_fields as $key => $value) {
                $prefixed_extension_fields[$key] = $value;
                $prefixed_extension_fields[$key]["attributes"]["id"] = "CnrAdditionalFields-" . $key;
            }

            $extension_fields = $this->arrayToModuleFields($prefixed_extension_fields, null, $vars);
            $whois_sections = array_merge($whois_sections, ["CnrAdditionalFields"]);
        }
    ...
    {
        CnrBase::setModule($this->getModuleRow($module_row_id));
        CnrBase::moduleInstance($this);
    ...
    private function manageSettings(
        ...
        if (isset($post["request_epp"])) {
            ...
            if (preg_match("/\.(de|be|no|eu)$/i", $fields->domain)) {
                ...
                CnrHelper::errorHandler($response);
    ...
    public function checkAvailability($domain, $module_row_id = null)
    {
        ...
        $r = $this->domainManager->call([
            ...
            // the necessary premium domain price data." Sending it on every check
            // is what makes the premium case visible early enough to act on.
        ...
        if ($dc === "210") { //AVAILABLE
            ...
            if (!empty($r["PROPERTY"]["X-FEE-CLASS"][0])
    ...
    //     CnrBase::setModule($this->getModuleRow($service->module_row_id ?? null));
    //     CnrBase::moduleInstance($this);
    ...
    /**
     * Verifies that the provided term is valid for the given TLD.
     *
     * Called by the order plugin for every domain being configured
     * (Blesta\Plugin\Order OrderTypeDomain::validateTerm). Returning false stops
     * the order, so the check only rejects a term the registry is known to
     * refuse - when no zone data is available the term is allowed through
     * rather than blocking a sale over missing metadata.
     *
     * Registration terms are intersected with the renewal terms because a Blesta
     * package term drives both the initial order and every renewal invoice. A
     * registry may accept a term for registration that it will not accept for
     * renewal - .com.ai registers up to ten years but renews at most nine - and
     * selling that term means the registration succeeds and the first renewal
     * fails. Where the two do not overlap at all the registration terms win, so
     * the TLD is not left unsellable.
     *
     * @param string $tld The TLD to check, with or without a leading dot
     * @param int $term The term in years
     * @param bool $transfer True if the term is for a transfer
    ...
    public function isValidTerm($tld, $term, $transfer = false)
    {
        ...
        // Blesta itself offers no term beyond 10 years
        if ($term > 10) {
            return false;
        }

        $row = $this->getModuleRow() ?? $this->getRow();
        if (empty($row)) {
            return true;
        }
        ...
        $zoneInfo = $this->domainManager->getZoneInfo("example." . ltrim((string) $tld, "."));
        if (!is_object($zoneInfo)) {
            return true;
        }

        if ($transfer) {
            $periods = (array) ($zoneInfo->transfer->periods ?? []);
        } else {
            $periods = (array) ($zoneInfo->registration->periods ?? []);
            $renewalPeriods = (array) ($zoneInfo->renewal->periods ?? []);
            if (!empty($renewalPeriods)) {
                $periods = array_intersect($periods, $renewalPeriods) ?: $periods;
            }
        }

        // Nothing known about this operation for the zone, do not block it
        if (empty($periods)) {
            return true;
        }

        return in_array($term, array_map("intval", $periods), true);
    }
    ...
    public function getFilteredTldPricing($module_row_id = null, $filters = [])
    {
        ...
        foreach ($tld_prices as $tld => $priceTypes) {
            ...
            foreach ($priceTypes as $pType => $priceValue) {
                if (!in_array($pType, CnrPriceType::billableTermValues(), true)) {
                    continue;
                }
                foreach ($priceValue["cost"] as $currency => $value) {
                    foreach ($priceValue["periods"] as $_ => $period) {
                        ...
                        if ($pType === "register") {
                            $setupFee = $priceTypes[CnrPriceType::Setup->value]["cost"][$currency] ?? 0;
                        }
    ...
    public function getTlds($module_row_id = null, $extended = false)
    {
        ...
        $cache = CnrHelper::hasCache($cacheKey, true);
        if ($cache) {
            return $cache;
        }
        ...
        if (!empty($r["PROPERTY"]["ZONE"])) {
            foreach ($r["PROPERTY"]["ZONE"] as $id => $zone) {
                ...
                $idns = CnrHelper::IDNConvert($tlds);
        ...
        if (count($convertIdnTlds) > 0) {
            CnrHelper::setCache($cacheKey, $convertIdnTlds);
        }
    ...
    public function manageEmailForwarding(
        ...
        if (!isset($response["CODE"]) || $response["CODE"] === "545") {
            ...
            CnrHelper::errorHandler($createDNS);
    ...
    private function manageDnsRecords(
        ...
        $vars->selects = array_combine(CnrHelper::getSupportedRRTypes(), array_values(CnrHelper::getSupportedRRTypes())); // make keys and values same
        $vars->records = CnrHelper::getResourceRecords($response["PROPERTY"] ?? [], $fields->domain);
    ...
    private function getTld($domain, $row = null)
    {
        ...
        // strstr() returns false for a name with no dot at all. Callers treat
        // the result as a string, so hand back an empty one instead.
    ...
    {
        // Fetch the TLDs results from the cache, if they exist
        $tldPricing = CnrHelper::hasCache("tld_pricing", true);

        Loader::loadModels($this, ['Currencies']);
        CnrBase::setModule($moduleRow);
        CnrBase::moduleInstance($this);
        ...
        if (empty($tldPricing)) {
            ...
            foreach ($tldsRawData as $tld => $tldData) {
                ...
                $tldPricing[$tld][CnrPriceType::Register->value]["cost"][$tldPriceCurrency] = (float) $tldData["pricing"]["annualFee"]; // ( anuual fee * term ) + setup fee
                $tldPricing[$tld][CnrPriceType::Register->value]["periods"] = $zoneInformations[$tldNoDot]->registration->periods;
                $tldPricing[$tld][CnrPriceType::Register->value]["defaultPeriod"] = $zoneInformations[$tldNoDot]->registration->defaultPeriod;
                $tldPricing[$tld][CnrPriceType::Renew->value]["cost"][$tldPriceCurrency] = (float) $tldData["pricing"]["annualFee"]; // ( anuual fee * term )
                $tldPricing[$tld][CnrPriceType::Renew->value]["periods"] = $zoneInformations[$tldNoDot]->renewal->periods;
                $tldPricing[$tld][CnrPriceType::Renew->value]["defaultPeriod"] = $zoneInformations[$tldNoDot]->renewal->defaultPeriod;
                $tldPricing[$tld][CnrPriceType::Transfer->value]["cost"][$tldPriceCurrency] = (float) $tldData["pricing"]["transferFee"]; // transfer fee
                $tldPricing[$tld][CnrPriceType::Transfer->value]["periods"] = $zoneInformations[$tldNoDot]->transfer->periods;
                $tldPricing[$tld][CnrPriceType::Transfer->value]["defaultPeriod"] = $zoneInformations[$tldNoDot]->transfer->defaultPeriod;
                $tldPricing[$tld][CnrPriceType::Redemption->value]["cost"][$tldPriceCurrency] = (float) $tldData["pricing"]["redemptionFee"];
                $tldPricing[$tld][CnrPriceType::Redemption->value]["days"] = $zoneInformations[$tldNoDot]->tld->redemptionDays;
                $tldPricing[$tld][CnrPriceType::Setup->value]["cost"][$tldPriceCurrency] = (float) $tldData["pricing"]["setupFee"];
            ...
            // Save the TLDs results to the cache
            CnrHelper::setCache("tld_pricing", $tldPricing);
        }
        ...
        foreach ($tldPricing as $tld => $pricing) {
            ...
            foreach ($pricing as $priceKey => $priceValue) {
                ...
                if (!in_array($priceKey, CnrPriceType::allValues(), true)) {
                    continue;
                }
